...
#    device = "cpu"
    ...
    train_loader = torch.utils.data.DataLoader(data_train, batch_size=1 ,shuffle = True, **kwargs)
    test_loader = torch.utils.data.DataLoader(data_test, batch_size=1 ,shuffle = False, **kwargs)
    print("-09-8080239",len(train_loader),len(test_loader),len(data_test))
    print("device: ",device)
    ...
    if (args.save_model):
        torch.save(model.state_dict(),"/home/nim/grad_cam/pt_files/file:mynet: train_acc:"+str(train_acc)+" test-acc:"+str(test_acc)+" epochs: "+str(args.epochs)+" UC_Merced.pt")
    # /home/nim/grad_cam/models
    save_name_pkl = "/home/nim/grad_cam/pickel_files/file:mynet: train_acc:"+str(train_acc)+" test-acc:"+str(test_acc)+" epochs: "+str(args.epochs)+" end.pkl"
    save_name_txt = "/home/nim/grad_cam/models/file:mynet: train_acc:"+str(train_acc)+" test-acc:"+str(test_acc)+" epochs: "+str(args.epochs)+" end.txt"
